...
/* ============================================================
   ONE-TIME DEDUPLICATION MIGRATION
   Collapses duplicate-titled pages and duplicate-named templates that
   accumulated in the local database from earlier cross-device syncs.
   Runs once per database (guarded by state.deduplicationVersion) and
   is idempotent: running it on an already-clean notebook is a no-op
   because no title-group will have more than one member.

   Must be called BEFORE the boot-time ensureXxx() functions so that
   the seeder guards (titleIndex checks) see the already-deduplicated
   state rather than orphaned duplicates.

   Counterpart to the real-time fix in mergeStates() above; together
   they cover both the existing data and all future syncs.
   ============================================================ */
var DEDUP_MIGRATION_VERSION = 1;

function deduplicateStateInPlace(s){
  if(!s || !s.pages || !s.blocks) return false;
  if((s.deduplicationVersion||0) >= DEDUP_MIGRATION_VERSION) return false;

  var changed = false;

  /* --- 1. Deduplicate pages by title ------------------------------------ */
  if(!s.tombstones) s.tombstones = {pages:{}, blocks:{}};
  if(!s.tombstones.pages) s.tombstones.pages = {};

  /* Build title → [pageId, ...] groups (live pages only). */
  var pageGroups = {};
  Object.keys(s.pages).forEach(function(pid){
    var p = s.pages[pid];
    if(!p || p.trashedAt) return;
    var key = (p.title||'').toLowerCase();
    if(!key) return;
    if(!pageGroups[key]) pageGroups[key] = [];
    pageGroups[key].push(pid);
  });

  Object.keys(pageGroups).forEach(function(key){
    var ids = pageGroups[key];
    if(ids.length < 2) return;

    /* Keep the page with the smallest createdAt (the original seed).
       Secondary sort on id for determinism. */
    ids.sort(function(a, b){
      var aAge = s.pages[a].createdAt||0;
      var bAge = s.pages[b].createdAt||0;
      return aAge - bAge || (a < b ? -1 : 1);
    });

    var winnerId = ids[0];
    var loserIds = ids.slice(1);

    loserIds.forEach(function(loserId){
      /* Reassign all blocks from loser to winner. */
      Object.keys(s.blocks).forEach(function(bid){
        var b = s.blocks[bid];
        if(b && b.pageId === loserId){
          b.pageId = winnerId;
          changed = true;
        }
      });
      ...
      /* Tombstone the loser. */
      var loserPage = s.pages[loserId];
      var tombTs = (loserPage && (loserPage.updatedAt||loserPage.createdAt))||Date.now();
      s.tombstones.pages[loserId] = Math.max(tombTs, s.tombstones.pages[loserId]||0);

      delete s.pages[loserId];
      changed = true;
    });
  });

  /* --- 2. Deduplicate templates by name --------------------------------- */
  if(s.templates && typeof s.templates === 'object'){
    var tmplGroups = {};
    Object.keys(s.templates).forEach(function(k){
      var t = s.templates[k];
      if(!t || typeof t.name !== 'string') return;
      var key = t.name.toLowerCase();
      if(!tmplGroups[key]) tmplGroups[key] = [];
      tmplGroups[key].push(k);
    });

    Object.keys(tmplGroups).forEach(function(key){
      var ids = tmplGroups[key];
      if(ids.length < 2) return;

      /* Keep the oldest (earliest createdAt). */
      ids.sort(function(a, b){
        var aAge = s.templates[a] ? (s.templates[a].createdAt||0) : 0;
        var bAge = s.templates[b] ? (s.templates[b].createdAt||0) : 0;
        return aAge - bAge || (a < b ? -1 : 1);
      });

      /* Delete all but the first (oldest). */
      ids.slice(1).forEach(function(k){
  ...
  /* --- 3. Stamp the migration version so this never runs again ---------- */
  s.deduplicationVersion = DEDUP_MIGRATION_VERSION;
  return changed; /* caller decides whether to persist immediately */
}
...
function initNotebook(){
  ...
  return loadAsync().then(function(loaded){
    ...
    state = loaded;
    /* Run the one-time deduplication migration FIRST — before any ensureXxx()
       boot seeder — so that title-index guards in ensureDocsPage() etc. see
       the already-deduplicated state and do not create a third copy of a page
       that still has two live entries from a previous sync. */
    deduplicateStateInPlace(state);
    ...
    clearTimeout(warnTimer);
    state = null;
    handleNotebookLoadFailure(err);
    var handled = new Error('notebook-load-failed');
    handled.nexusHandled = true;
    throw handled;
  });
}

/* ---- Load-failure protection ------------------------------------------------
   If a notebook IS stored but cannot be read (IndexedDB error, corrupt JSON,
   encrypted record with no key, unrecognised shape) Nexus must not quietly
   start a blank notebook — the first autosave would overwrite the real data.
   Instead writes are blocked and the person chooses what to do. */
function readRawNotebookRecord(){
...
function rawRecordToText(raw){ return typeof raw === 'string' ? raw : JSON.stringify(raw); }
function handleNotebookLoadFailure(err){
  notebookWriteBlocked = true;
  try{ console.error('Nexus could not load the stored notebook:', err); }catch(ignore){}
  var old = document.getElementById('load-failure-overlay');
  if(old && old.parentNode) old.parentNode.removeChild(old);
  var reason = (err && err.message) ? String(err.message) : 'unknown error';
  var explain = reason === 'locked'
    ? 'Your notebook is encrypted, but this browser no longer has the passcode information needed to open it (site data may have been cleared).'
    : 'Nexus found your stored notebook but could not read it (' + reason + ').';

  var overlay = document.createElement('div');
  overlay.id = 'load-failure-overlay';
  overlay.setAttribute('role', 'alertdialog');
  overlay.setAttribute('aria-modal', 'true');
  overlay.setAttribute('aria-labelledby', 'load-failure-title');
  var card = document.createElement('div');
  card.className = 'load-failure-card';
  function add(tag, text, cls){ var el = document.createElement(tag); if(cls) el.className = cls; el.textContent = text; card.appendChild(el); return el; }
  add('h2', 'Your notebook could not be opened', 'load-failure-title').id = 'load-failure-title';
  add('p', explain);
  add('p', 'Nothing has been changed or deleted, and Nexus has paused saving so it cannot overwrite what is stored. Try again first — most problems are temporary. If it keeps failing, save a copy of the unreadable data, then either restore a backup or start a fresh notebook.');
  var msg = add('p', '', 'load-failure-msg');
  var row = document.createElement('div');
  row.className = 'load-failure-actions';
  ...
  button('Try again', true, function(){ location.reload(); });
  button('Save a copy of the stored data', false, function(){
    readRawNotebookRecord().then(function(raw){
      if(raw == null){ msg.textContent = 'There is no stored notebook record to copy.'; return; }
      downloadTextFile('nexus-unreadable-notebook-' + Date.now() + '.json', rawRecordToText(raw));
      msg.textContent = 'A copy was downloaded. Encrypted notebooks stay encrypted in that file.';
    }).catch(function(e){ msg.textContent = 'Could not read the stored data: ' + (e && e.message ? e.message : e); });
  });
  button('Start a fresh notebook', false, function(){
    if(!confirm('Start a fresh, empty notebook?\n\nThe unreadable data is first copied to a file and kept inside this browser\'s storage, but it will no longer be what Nexus opens. You can restore a backup afterwards from Settings.')) return;
    readRawNotebookRecord().catch(function(){ return null; }).then(function(raw){
      if(raw == null) return null;
      downloadTextFile('nexus-unreadable-notebook-' + Date.now() + '.json', rawRecordToText(raw));
      return openAttachmentDb().then(function(db){
        return new Promise(function(resolve, reject){
          var tx = db.transaction(NB_STORE, 'readwrite');
          tx.objectStore(NB_STORE).put(raw, 'state_quarantine_' + Date.now());
          tx.oncomplete = function(){ resolve(); };
          tx.onerror = tx.onabort = function(){ reject(tx.error || new Error('quarantine write failed')); };
        });
      });
    }).then(function(){
      notebookWriteBlocked = false;
      forceFreshNotebook = true;
      if(overlay.parentNode) overlay.parentNode.removeChild(overlay);
      bootNotebook();
    }).catch(function(e){
      msg.textContent = 'Could not keep a copy of the unreadable data, so nothing was changed: ' + (e && e.message ? e.message : e);
    });
  });
...
function normalizeState(parsed){
  ...
  /* Preserve the deduplication migration stamp so re-normalisation (restore,
     merge) never resets it and triggers the migration a second time. */
  if(typeof parsed.deduplicationVersion !== 'number') parsed.deduplicationVersion = 0;
...
/* Strict: throws if the stored record cannot be turned into a notebook.
   It must NEVER fall back to seedState() for a record that exists — a
   fresh seed would be persisted straight over the person's real (merely
   unreadable) data. Only "nothing stored at all" may seed; see loadAsync(). */
function parseLoadedState(raw){
  var parsed = (typeof raw === 'string') ? JSON.parse(raw) : raw;
  if(!parsed || typeof parsed !== 'object' || !parsed.pages || typeof parsed.pages !== 'object' ||
     !parsed.blocks || typeof parsed.blocks !== 'object'){
    throw new Error('The stored notebook is not in a recognisable format.');
  }
...
var forceFreshNotebook = false; /* set only by the explicit "Start a fresh notebook" choice after a load failure */
function loadAsync(){
  if(forceFreshNotebook){ forceFreshNotebook = false; return Promise.resolve(seedState()); }
  return getNotebookState().then(function(raw){
    /* A literal "null" record is what an older build wrote when it flushed before the
       notebook had loaded; there is nothing recoverable in it, so treat it as empty. */
    if(raw && !(typeof raw === 'string' && raw.trim() === 'null')) return parseLoadedState(raw);
    var legacy = null;
    try{ legacy = localStorage.getItem(STORAGE_KEY); }catch(e){}
    if(!legacy) return seedState(); /* genuinely nothing stored: a brand-new notebook */
    var legacyState = parseLoadedState(legacy); /* throws (and keeps the localStorage copy) if unreadable */
    return putNotebookState(legacy).then(function(){
      try{ localStorage.removeItem(STORAGE_KEY); }catch(e){}
      return legacyState;
    }).catch(function(){ return legacyState; }); /* keep the localStorage copy if the IndexedDB write failed */
  });
  /* No .catch here on purpose: a read/decrypt/parse failure is reported by
     initNotebook() -> handleNotebookLoadFailure(), never turned into a blank notebook. */
}
...
var notebookWriteBlocked = false; /* true while a stored notebook exists but could not be read: nothing may overwrite it */
function looksLikeNotebookJson(json){
  return typeof json === 'string' && json.charAt(0) === '{' && json.indexOf('"pages"') !== -1 && json.indexOf('"blocks"') !== -1;
}
function enqueueNotebookPersist(json, keyOverride){
  if(notebookWriteBlocked){
    return Promise.reject(new Error('Saving is paused: the stored notebook could not be read, so Nexus will not overwrite it.'));
  }
  if(!looksLikeNotebookJson(json)){
    return Promise.reject(new Error('Refusing to save an empty or invalid notebook.'));
  }
...
/* A line being typed lives only in its contenteditable DOM until it is
   committed (blur / Enter / indent...). Anything that snapshots `state`
   (autosave, flush on hide/unload, sync merge, lock) must first pull that
   text in, without leaving edit mode or disturbing the caret. Returns true
   if state changed. */
function syncEditingBlockToState(){
  ...
  var row = el.closest ? el.closest('.block-row') : null;
  var b = row && state.blocks[row.dataset.id];
  if(!b) return false;
  var text;
  try{ text = serializeInline(el); }catch(e){ return false; }
  if(typeof text !== 'string' || text === b.text) return false;
  b.text = text;
  return true;
}
/* Debounced autosave while typing: keeps the newest text in state and on
   disk within ~0.7s of the last keystroke. It deliberately records no undo
   checkpoint (the commit that follows does) and does not broadcast. */
var editAutosaveTimer = null;
var EDIT_AUTOSAVE_MS = 700;
function scheduleEditAutosave(){
  ...
  editAutosaveTimer = setTimeout(function(){
    ...
    if(syncEditingBlockToState()) save({recordUndo:false, broadcast:false});
  }, EDIT_AUTOSAVE_MS);
}
...
function flushSaveNow(){
  clearTimeout(saveTimer);
  /* Never persist a notebook that has not loaded (or failed to load): JSON.stringify(null)
     is "null", which would replace the person's whole notebook. */
  if(!state || typeof state !== 'object' || !state.pages) return Promise.resolve(false);
  if(syncEditingBlockToState()){ try{ touchChangedEntities(); }catch(e){} }
  var json;
...
/* Templates carry no updatedAt and aren't tombstoned (they're simple
   ...
   DEDUP FIX: After the UUID union, collapse same-named templates into a
   single canonical entry. When two devices each seed "Meeting notes" /
   "Daily journal" independently they get different UUIDs; the union
   would otherwise produce duplicate sidebar entries. We keep the entry
   with the earlier createdAt (preserving the "original" seed timestamp)
   and discard the other. User-created templates with unique names are
   unaffected. */
function mergeTemplateMaps(a, b){
  ...
  /* Collapse duplicates by (lower-cased) name: keep the one with the
     earlier createdAt so the "original" seed is the survivor. */
  var byName = {};
  Object.keys(out).forEach(function(k){
    ...
    if(!byName[key]){
      ...
      var existingT = out[byName[key]];
      var existingAge = existingT ? (existingT.createdAt||0) : 0;
      var candidateAge = t.createdAt||0;
      if(candidateAge < existingAge){
        /* candidate is older — it becomes the survivor, drop existing */
        delete out[byName[key]];
        byName[key] = k;
      } else {
        /* existing is older or equal — drop candidate */
        delete out[k];
      }
    }
  });
...
function mergeStates(local, remote, sinceTs){
  ...
  var merged = {
    ...
    /* Carry the dedup migration stamp forward: take the higher of the two so
       a device that already ran the migration stays migrated after syncing
       with a device that hasn't yet. */
    deduplicationVersion: Math.max(local.deduplicationVersion||0, remote.deduplicationVersion||0),
    flashcards: {decks:{}, cards:{}},
    stickyNotes: {cards:{}},
    folders: {},
  ...
  /* ---- TITLE-BASED PAGE DEDUPLICATION ----------------------------------------
     After the UUID-keyed merge, two live pages with the same title can exist when
     two devices independently seeded the same logical entity (e.g. "Database",
     "Queries", "Help & Tutorial", "Welcome to Nexus") and each produced a
     different UUID. Neither UUID appears in the other device's page map, so the
     per-UUID loop above keeps both. This pass collapses them into one canonical
     record before the sibling arrays are rebuilt.

     Algorithm:
       1. Group all non-trashed merged pages by lower-cased title.
       2. For each group with more than one member, pick the canonical winner:
            a. Prefer the page already known to local state (stable local UUID).
            b. Otherwise prefer the page with the earlier createdAt (original seed).
       3. Re-assign every block belonging to a loser page to the winner's pageId.
       4. Tombstone the loser page so future syncs never re-introduce it.
       5. Remove the loser from merged.pages.

     Safety constraints:
       - Only collapses pages whose titles are identical after lower-casing.
       - Never collapses trashed pages.
       - Never removes content: all blocks from the loser are adopted by the winner.
       - Only runs on the freshly merged state, not on the persisted local state
         directly — so a user-deleted duplicate is not recreated.
  --------------------------------------------------------------------------- */
  (function deduplicatePagesByTitle(){
    /* Group live (non-trashed) pages by lower-cased title. */
    var groups = {}; /* titleKey -> [pageId, ...] */
    Object.keys(merged.pages).forEach(function(pid){
      var p = merged.pages[pid];
      if(!p || p.trashedAt) return;
      var key = (p.title||'').toLowerCase();
      if(!key) return;
    ...
    Object.keys(groups).forEach(function(key){
      var ids = groups[key];
      if(ids.length < 2) return; /* no duplicate — nothing to do */

      /* Pick canonical winner: prefer a page already present in local.pages
         (stable local UUID over newly-arrived remote UUID). Break ties by
         createdAt ascending (keep the oldest / original seed). */
      ids.sort(function(a, b){
        var aLocal = !!(local.pages && local.pages[a]);
        var bLocal = !!(local.pages && local.pages[b]);
        if(aLocal !== bLocal) return aLocal ? -1 : 1; /* local first */
        var aAge = merged.pages[a].createdAt||0;
        var bAge = merged.pages[b].createdAt||0;
      ...
      loserIds.forEach(function(loserId){
        /* Re-parent every block that was on the loser page to the winner page. */
        Object.keys(merged.blocks).forEach(function(bid){
        ...
        /* Tombstone the loser so it is never re-introduced by future syncs.
           Use the loser's own updatedAt (or now) as the tombstone timestamp,
           which is >= any of its blocks' updatedAt values. */
        var loserPage = merged.pages[loserId];
        var tombTs = (loserPage && loserPage.updatedAt) || Date.now();
        merged.tombstones.pages[loserId] = Math.max(tombTs, merged.tombstones.pages[loserId]||0);

        /* Remove the loser page from the merged state. */
        delete merged.pages[loserId];

        /* Keep pageWinner map consistent so the sibling-rebuild below
           picks up blocks from the right container. */
        delete pageWinner[loserId];
      });
    });
  })();
  /* ---- END TITLE-BASED PAGE DEDUPLICATION --------------------------------- */
...
function applyIncomingMerge(merged){
  ...
  var resumeId = editingBlockId, resumeOffset = 0;
  try{
    var liveEl = document.querySelector('.block-content.editing');
    if(resumeId && liveEl && typeof getCaretOffset === 'function') resumeOffset = getCaretOffset(liveEl);
  }catch(e){ resumeOffset = 0; }
  editingBlockId = null;
  dockBlockId = null;
  state = merged;
  ...
  renderAll();
  /* A background sync must not throw the person out of the line they are typing in. */
  if(resumeId && state.blocks[resumeId] && typeof focusBlock === 'function'){
    try{ focusBlock(resumeId, resumeOffset); }catch(e){}
  }
}
...
function escapeHtml(str){
  /* Total function: numbers (footnote labels, counts), null and undefined must never throw. */
  ...
  node.childNodes.forEach(function(n){
    if(n.nodeType === 3){
      ...
      if(n.classList && n.classList.contains('link')){
        ...
        out += '((' + (n.dataset.refid || '') + '))';
      } else if(n.classList && n.classList.contains('footnote-ref')){
